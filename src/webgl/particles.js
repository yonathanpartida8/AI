...
export function mountParticles(canvas, opts = {}) {
  ...
  function step() {
    ...
    if (!visible) return; // render bajo demanda: no gasta GPU fuera de pantalla
    t += 0.016 * speed;
    const cx = W() / 2, cy = H() / 2;
    for (let i = 0; i < count; i++) {
      if (mode === 'órbita') {
        orbit[i * 2 + 1] += 0.004 * speed * (1 + (i % 5) * 0.15);
