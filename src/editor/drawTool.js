...
export class DrawTool {
  ...
  #buildToolbar() {
    ...
    const bar = el('div', { class: 'draw-toolbar', style: { display: 'none' } }, [
      el('span', { class: 'draw-title', html: `${ic('pen')}<span>Libreta</span>` }),
      el('select', {
      ...
      el('button', { class: 'btn', html: `${ic('square')}<span>Borrador</span>`, onclick: (e) => { this.brush.eraser = !this.brush.eraser; e.target.classList.toggle('active', this.brush.eraser); } }),
      photoInput,
      el('button', { class: 'btn', html: `${ic('image')}<span>Foto</span>`, title: 'Añade una fotografía de tu galería al dibujo', onclick: () => photoInput.click() }),
      el('button', { class: 'btn btn-ic', html: ic('undo'), title: 'Deshacer trazo', onclick: () => this.undoStroke() }),
      el('button', { class: 'btn btn-ic', html: ic('redo'), title: 'Rehacer trazo', onclick: () => this.redoStroke() }),
      el('button', { class: 'btn', html: `${ic('trash')}<span>Limpiar</span>`, onclick: () => this.clear() }),
      el('button', { class: 'btn', html: `${ic('download')}<span>PNG</span>`, title: 'Descargar la hoja', onclick: () => this.downloadPNG() }),
      el('button', { class: 'btn primary', html: `${ic('check')}<span>Convertir en componente</span>`, onclick: () => this.toComponent() }),
      el('button', { class: 'btn btn-ic', html: ic('close'), title: 'Salir', onclick: () => this.store.setTool('select') }),
    ]);
    return bar;
  }
}
