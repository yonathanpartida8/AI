...
async function boot() {
  // Paleta elegida por el usuario (se recuerda entre sesiones).
  // Aquí no hay modo oscuro: el editor es siempre claro y pastel.
  const paleta = localStorage.getItem('wb-palette');
  if (paleta) document.body.dataset.palette = paleta;
  localStorage.removeItem('wb-theme');   // resto de la versión con temas
...
/**
 * EL TECLADO VIRTUAL NO PUEDE TAPAR LO QUE ESCRIBES.
 *
 * Al abrirse, el teclado encoge el viewport VISUAL pero no el de
 * maqueta. Como los cajones están anclados con `position: fixed` al
 * fondo de la pantalla, se quedaban detrás del teclado: escribías a
 * ciegas. La API de visualViewport dice cuánto ocupa, y esa medida
 * viaja a la hoja de estilos en --teclado.
 */
function vigilarTecladoVirtual() {
  ...
  const ajustar = () => {
    ...
    if (tapado === ultimo) return;               // ni una escritura de más
    ultimo = tapado;
    raiz.style.setProperty('--teclado', `${tapado}px`);
    // Por debajo de 120 px es la barra del navegador, no un teclado
    document.body.classList.toggle('teclado-abierto', tapado > 120);
  };
  vv.addEventListener('resize', ajustar, { passive: true });
  vv.addEventListener('scroll', ajustar, { passive: true });
  ajustar();

  // Y el campo que acabas de tocar se coloca a la vista
  document.addEventListener('focusin', (e) => {
    const campo = e.target;
    if (!/^(INPUT|TEXTAREA|SELECT)$/.test(campo.tagName)) return;
    setTimeout(() => {
      if (document.activeElement !== campo) return;
      campo.scrollIntoView({ block: 'center', behavior: 'smooth' });
    }, 280);   // lo que tarda el teclado en subir
  });
}
